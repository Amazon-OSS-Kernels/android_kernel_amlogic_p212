...
#ifdef CONFIG_SECURITY_SELINUX_ALLOW_PERMISSIVE
	/* To get user build working with SELinux in permissive mode,
	we need to overwrite this value only if uboot is passing permissive
	mode to the Kernel command line*/
	if (strstr(saved_command_line, "androidboot.selinux=permissive") != NULL)
		new_value = 0;
#endif
