 ...
 */
static int snd_soc_read_signed(struct snd_soc_codec *codec,
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = snd_soc_read_signed(codec, reg, mask, shift, sign_bit, &val);
	if (ret)
		return ret;
	...
	if (snd_soc_volsw_is_stereo(mc)) {
		if (reg == reg2)
			ret = snd_soc_read_signed(codec, reg, mask, rshift,
				sign_bit, &val);
		else
			ret = snd_soc_read_signed(codec, reg2, mask, shift,
				sign_bit, &val);
		if (ret)
			return ret;
