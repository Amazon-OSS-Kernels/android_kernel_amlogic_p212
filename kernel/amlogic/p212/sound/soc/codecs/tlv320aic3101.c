...
static const struct adc31xx_rate_divs aic31xx_divs_pll[] = {
 ...
 //	{ 12288000, 16000,     8,     8,     8,     0,   16,    3,  128,      1 }, /* PLL enable case. mclk*r*(j.d)/p/nadc/bdiv_n = 6144000 */
	{ 12288000, 16000,     1,     1,     1,     0,    2,    3,  128,      1 }, /* PLL disable case. same formula above */
	{ 24576000, 16000,     1,     1,     1,     0,   4,     3,  128,      1 }, /* PLL disable case, 12.288*2 * 1*1/1/4/1 = 6144000 */
};

/* master adc generated bit clock */
static const struct adc31xx_rate_divs aic31xx_divs_bclk[] = {
	...
	{ 6144000,  16000,     5,     1,    48,     0,    1,    3,  128,      1 }, /* pll[] matched with this line */
...
/*
 *----------------------------------------------------------------------------
 * Function : aic31xx_change_page
 * Purpose  : This function is to switch between page 0 and page 1.
 *
 *----------------------------------------------------------------------------
 */
static int aic31xx_change_page(unsigned int device, struct snd_soc_codec *codec,
			       u8 new_page)
{
	...
	dev_dbg(codec->dev, "AIC dev %#x change page %d\n", device, new_page);

	data[0] = 0x0;
	data[1] = new_page;


	if (i2c_master_send(aic31xx->adc_control_data[device], data, 2) != 2) {
		dev_err(codec->dev, "aic31xx_change_page %d: I2C Wrte Error\n",
			device);
		return -1;
	}
	aic31xx->adc_page_no[device] = new_page;
	return 0;
}
			 ...
			 unsigned int value)
{
	...
	if (aic31xx->adc_page_no[device] != page)
		aic31xx_change_page(device, codec, page);
...
static void tlv320aic3101_sw_reset(struct snd_soc_codec *codec)
{
	...
	snd_soc_cache_init(codec); // Need to set cache to initial value
	msleep(20);
}
...
{
	int i, mic_pos = 1; /* use 1 here, while abh123 uses 0 */
	struct aic31xx_priv *aic31xx = snd_soc_codec_get_drvdata(codec);
#ifdef CONFIG_ACPI
	...
	if (mic_pos == 0) {
		...
		u8 mic_channelmap[] = { /* for ADC A/B/C/D: left channel, right channel. 1 to 8 */
			0x07, 0x08, 0x05, 0x04,
			0x01, 0x02, 0x03, 0x06
		};
		for (i = 0; i < NUM_ADC3101; i++) {
			u8 enable_channels = ~(aic31xx->tdm_rx_mask >> (i * 2)) & ADC_TDM_MASK;
			u8 left  = mic_channelmap[2*i];
			u8 right = mic_channelmap[2*i + 1];

			// use this to only turn on one ADC for debugging: if (i!=3) enable_channels=3;
			if (left > right) {
				snd_soc_write(codec, ADC_CH_OFFSET_1(i), (right - 1 )* aic31xx->tdm_slot_width | aic31xx->dsp_a_val);
				snd_soc_write(codec, ADC_CH_OFFSET_2(i), (left - right - 1) * aic31xx->tdm_slot_width);
				snd_soc_update_bits(codec, ADC_I2S_TDM_CTRL(i),
						0x1 << ADC_TDM_CHANNEL_SWAP_SHIFT,
						0x1 << ADC_TDM_CHANNEL_SWAP_SHIFT);
			}
			else {
				snd_soc_write(codec, ADC_CH_OFFSET_1(i), (left - 1) * aic31xx->tdm_slot_width | aic31xx->dsp_a_val);
				snd_soc_write(codec, ADC_CH_OFFSET_2(i), (right - left - 1) * aic31xx->tdm_slot_width);
			}
...
static void tlv320aic3101_adc_cfg(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < NUM_ADC3101; i++) {
		/* Differential 0db setup*/
		snd_soc_write(codec, ADC_LEFT_PGA_SEL_1(i), DIFF_MIC_INPUT_0db);
		snd_soc_write(codec, ADC_RIGHT_PGA_SEL_1(i), DIFF_MIC_INPUT_0db);

		/* Setup PGA gain */
		snd_soc_write(codec, ADC_LEFT_APGA_CTRL(i), MIC_PGA_GAIN_L);
		snd_soc_write(codec, ADC_RIGHT_APGA_CTRL(i), MIC_PGA_GAIN_R);
...
static struct snd_soc_dai_driver tlv320aic3101_dai_driver[] = {
{
	...
	.capture = {
		.stream_name	= "Capture",
		.channels_min	= 1,
#if defined(CONFIG_SND_SOC_7_MICS)
		.channels_max	= 9,
#elif defined(CONFIG_SND_SOC_8_MICS)
		.channels_max	= 10,
#else
		.channels_max	= 4,
#endif
...
//		aic31xx->sysclk = DEFAULT_MCLK;

#if 1
		aic31xx->mclk = 0; // clock already enabled in machine driver. no need to do below
#else
		aic31xx->mclk = devm_clk_get(&pdev->dev, "mclk");
		if (IS_ERR(aic31xx->mclk)) {
			dev_err(&pdev->dev, "%s: Failed getting mclk\n", __func__);
			aic31xx->mclk = 0; //assign 0, in case we disable the return kernel won't crash
