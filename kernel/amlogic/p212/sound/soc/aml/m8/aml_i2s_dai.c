...
extern int i2s_dac_state;
extern int64_t i2s_dac_timestamp;
			       ...
			       struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			struct timespec ts;
			getrawmonotonic(&ts);
			i2s_dac_timestamp = timespec_to_ns(&ts);
			i2s_dac_state = 1;
				 ...
				 struct snd_soc_dai *dai)
{
	...
	if (i2s->old_samplerate != srate) {
		...
		ratio = DEFAULT_MCLK / (BCLK_FS_RATIO * srate);
		pr_info("i2s mclk to sclk ratio = %d, SR = %d\n", ratio, srate);
		audio_set_i2s_clk_div(ratio);
	}
...
static int aml_i2s_dai_probe(struct platform_device *pdev)
{
	...
	/* set default amclk */
	ret = aml_i2s_set_amclk(i2s, DEFAULT_MCLK);
	if (ret < 0) {
		dev_err(&pdev->dev, "Can't set aml_i2s :%d\n", ret);
		goto err;
	}
	ratio = DEFAULT_MCLK / (BCLK_FS_RATIO * DEFAULT_SAMPLERATE);
	/* set default I2S clks */
	audio_set_i2s_clk_div(ratio);
