...
#include "../../../drivers/misc/amz_priv.h"
#endif
...
/* mclk fixed to DEFAULT_MCLK, but adaptable mclk/sclk ratio. */
void audio_set_i2s_clk_div(unsigned int ratio)
{
	/* aiclk source */
	aml_cbus_update_bits(AIU_CLK_CTRL, 1 << 10, 1 << 10);
	/* Set mclk over sclk ratio */
	aml_cbus_update_bits(AIU_CLK_CTRL_MORE, 0x3f, (ratio - 1));
	aml_cbus_update_bits(AIU_CLK_CTRL_MORE, 0x3f << 8, (ratio - 1) << 8);
...
{
	aml_cbus_update_bits(AIU_HDMI_CLK_DATA_CTRL, 0x3, source);
	aml_cbus_update_bits(AIU_HDMI_CLK_DATA_CTRL, 0x3 << 4, source << 4);
}

extern int idme_get_board_long_rev(void);
int audio_hw_getmclk(void)
{
#ifdef CONFIG_AMZ_PRIV
        if (ishvt(idme_get_board_long_rev())) {
		pr_info("MCLK for HVT\n");
                return DEFAULT_MCLK_12M;
        }
	else
#endif
	{
                pr_info("MCLK for other build\n");
                return DEFAULT_MCLK_24M;
	}
}
