...
extern int spdif_dac_state;
extern int64_t spdif_dac_timestamp;
				 ...
				 struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			struct timespec ts;
			getrawmonotonic(&ts);
			spdif_dac_timestamp = timespec_to_ns(&ts);
			spdif_dac_state = 1;
		} else {
...
void aml_hw_iec958_init(struct snd_pcm_substream *substream, int samesrc)
{
	...
	IEC958_mode_codec == 7 || IEC958_mode_codec == 8) {
		...
		uint div;
		pr_info("share the same clock\n");
		div = DEFAULT_MCLK / (runtime->rate * BCLK_FS_RATIO * 2);
		if (div < 1) {
			pr_err("%s samplerate don't supported", __func__);
			return;
		}
		aml_cbus_update_bits(AIU_CLK_CTRL, 3 << 4, (div - 1) << 4);
	} else {
		pr_info("set normal 512 fs /4 fs\n");
		aml_cbus_update_bits(AIU_CLK_CTRL, 3 << 4, 3 << 4);
	}
...
static int aml_dai_spdif_probe(struct platform_device *pdev)
{
	...
	if (!spdif_priv) {
        ...
        dev_set_name(&pdev->dev,"aml-spdif");
