...
extern struct snd_soc_ops tlv3101_machine_ops;
extern struct snd_soc_ops tlv32x4_machine_ops;
			  ...
			  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!IS_ERR(p_audio->mute_spkr_desc))
		gpiod_direction_output(p_audio->mute_spkr_desc, aml_m8_spk_enabled);
...
static int aml_suspend_pre(struct snd_soc_card *card)
{
	...
	if (!IS_ERR(p_aml_audio->mute_spkr_desc)) {
		val = p_aml_audio->mute_spkr_inv ?
			GPIOF_OUT_INIT_LOW : GPIOF_OUT_INIT_HIGH;
		gpiod_direction_output(p_aml_audio->mute_spkr_desc, val);
	};
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		val = p_audio->mute_spkr_inv ?
			GPIOF_OUT_INIT_LOW : GPIOF_OUT_INIT_HIGH;
		gpiod_direction_output(p_audio->mute_spkr_desc, val);
		...
		struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_sysclk(codec_dai, 0,
			DEFAULT_MCLK, /*params_rate(params) * DEFAULT_MCLK_RATIO_SR */
			SND_SOC_CLOCK_IN);
	if (ret < 0) {
...
static void aml_m8_pinmux_init(struct snd_soc_card *card)
{
	...
	p_aml_audio = snd_soc_card_get_drvdata(card);
	p_aml_audio->mute_spkr_desc = gpiod_get(card->dev, "mute_spkr");
	p_aml_audio->mute_spkr_inv =
	    of_property_read_bool(card->dev->of_node, "mute_spkr_inv");
	if (!IS_ERR(p_aml_audio->mute_spkr_desc)) {
		if (p_aml_audio->sleep_time)
			msleep(p_aml_audio->sleep_time);
		val = p_aml_audio->mute_spkr_inv ?
	...
	p_aml_audio->mute_mic_desc = gpiod_get(card->dev, "mute_mic");
	p_aml_audio->mute_mic_inv =
	    of_property_read_bool(card->dev->of_node, "mute_mic_inv");
	if (!IS_ERR(p_aml_audio->mute_mic_desc)) {
		val = p_aml_audio->mute_mic_inv ?
			GPIOF_OUT_INIT_HIGH : GPIOF_OUT_INIT_LOW;
		gpiod_direction_output(p_aml_audio->mute_mic_desc, val);
	}
	if (is_jtag_apao())
		return;
...
static int aml_card_dais_parse_of(struct snd_soc_card *card)
{
	...
	int i, ret;
	int (*init)(struct snd_soc_pcm_runtime *rtd);
	struct snd_soc_ops *pmachine_ops = NULL;
...
#ifdef CONFIG_SND_SOC_TLV320AIC32X4
		if (i == 0)
			pmachine_ops = &tlv32x4_machine_ops;
#endif
#ifdef CONFIG_SND_SOC_TLV320AIC3101
		if (i == 2)
			pmachine_ops = &tlv3101_machine_ops;
#endif

		ret =
		    aml_card_dai_parse_of(dev, &dai_links[i], init, pmachine_ops, cpu_node,
					  codec_node, plat_node);

		/* need to return err immediately, otherwise ret will be overwritten */
		if (ret) break;
	}
