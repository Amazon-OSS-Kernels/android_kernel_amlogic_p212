...
static bool aml_i2s_alsa_output_running;
static bool aml_spdif_alsa_output_running;
#endif

unsigned long aml_i2s_alsa_write_addr = 0;
EXPORT_SYMBOL(aml_i2s_alsa_write_addr);

unsigned long aml_spdif_alsa_write_addr = 0;
EXPORT_SYMBOL(aml_spdif_alsa_write_addr);
...
int i2s_dac_state=0;
int64_t i2s_dac_timestamp=0;
int spdif_dac_state=0;
int64_t spdif_dac_timestamp=0;
...
static unsigned int period_sizes[] = {
	...
	12288, 16384, 24576, 32768, 49152, 65536, 65536 * 2, 65536 * 4
};
...
static int aml_i2s_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			if (s->device_type == AML_AUDIO_I2SOUT)
				aml_i2s_alsa_output_running = true;
			else
				aml_spdif_alsa_output_running = true;
		}
#endif
	...
	case SNDRV_PCM_TRIGGER_STOP:
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			if (s->device_type == AML_AUDIO_I2SOUT)
				aml_i2s_alsa_output_running = false;
			else
				aml_spdif_alsa_output_running = false;
		}
#endif
		break;
	default:
				 ...
				 struct snd_pcm_substream *substream)
{
	...
	if (s->device_type == AML_AUDIO_I2SOUT)
		aml_i2s_alsa_write_addr = offset + n;
	else
		aml_spdif_alsa_write_addr = offset + n;
...
static int aml_audio_latency_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
...
static int aml_audio_set_hrt(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
	return 0;
}

static int aml_audio_get_hrt(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer64.value[0] = hrt;
	return 0;
}

static int aml_audio_get_i2s_dac_state(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = i2s_dac_state;
	return 0;
}

static int aml_audio_get_i2s_dac_timestamp(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer64.value[0] = i2s_dac_timestamp;
	return 0;
}

static int aml_audio_get_spdif_dac_state(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = spdif_dac_state;
	return 0;
}

static int aml_audio_get_spdif_dac_timestamp(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer64.value[0] = spdif_dac_timestamp;
	return 0;
}

static int aml_audio_set_dai_timestamp(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int aml_audio_get_latency(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	unsigned long flags;
	unsigned int reg_rd_ptr, reg_start, reg_end;
#ifndef CONFIG_SND_AML_SPLIT_MODE
	unsigned int reg_clk = 0;
#endif
	unsigned int reg_hrt_low, reg_hrt_high, wr_ptr;
	int level, size;
	...
	if (reg == 0) {
		reg_start = aml_read_cbus(AIU_MEM_I2S_START_PTR);
		reg_end = aml_read_cbus(AIU_MEM_I2S_END_PTR);
		reg_rd_ptr = aml_read_cbus(AIU_MEM_I2S_RD_PTR);
#ifndef CONFIG_SND_AML_SPLIT_MODE
		reg_clk = aml_read_cbus(AIU_CLK_CTRL_MORE);
#endif
	} else {
		reg_start = aml_read_cbus(AIU_MEM_IEC958_START_PTR);
		reg_end = aml_read_cbus(AIU_MEM_IEC958_END_PTR);
		reg_rd_ptr = aml_read_cbus(AIU_MEM_IEC958_RD_PTR);
	}
	reg_hrt_low  = (uint32_t)aml_read_cbus(ISA_TIMERE);
	reg_hrt_high = (uint32_t)aml_read_cbus(ISA_TIMEREH);
	hrt = ((uint64_t)reg_hrt_high << 32) | reg_hrt_low;

	local_irq_restore(flags);

	if (reg == 0) {
#ifdef CONFIG_SND_AML_SPLIT_MODE
		size = reg_end - reg_start + 256;
#else
		size = reg_end - reg_start + ((reg_clk & (1 << 6)) ? 256 : 64);
#endif
		wr_ptr = reg_start + aml_i2s_alsa_write_addr % size;
	} else {
		size = reg_end - reg_start + 8;
		wr_ptr = reg_start + aml_spdif_alsa_write_addr % size;
	}

	if ((reg == 0) && !aml_i2s_alsa_output_running)
		level = 0;
	else if ((reg == 1) && !aml_spdif_alsa_output_running)
		level = 0;
	else if (wr_ptr == reg_rd_ptr)
		level = size;
	else if (wr_ptr > reg_rd_ptr)
		level = wr_ptr - reg_rd_ptr;
	else
		level = wr_ptr - reg_rd_ptr + size;
...
static const struct snd_kcontrol_new aml_i2s_controls[] = {
...
#ifndef CONFIG_SND_AML_SPLIT_MODE_MMAP
	{
		...
		.name = "I2S Latency",
		.info = aml_audio_latency_info,
		.get = aml_audio_get_latency,
		.private_value = 0,
	...
	},
	{
		...
		.name = "I2S DAI State",
		.info = aml_audio_latency_info,
		.get = aml_audio_get_i2s_dac_state,
		.private_value = 1,
		.access = SNDRV_CTL_ELEM_ACCESS_READ,
	},
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name = "SPDIF DAI State",
		.info = aml_audio_latency_info,
		.get = aml_audio_get_spdif_dac_state,
		.private_value = 1,
		.access = SNDRV_CTL_ELEM_ACCESS_READ,
	},
	SND_SOC_BYTES_EXT("HRT", BYTES_OF_INT64, aml_audio_get_hrt,
				aml_audio_set_hrt),
	SND_SOC_BYTES_EXT("I2S DAI Timestamp", BYTES_OF_INT64, aml_audio_get_i2s_dac_timestamp,
				aml_audio_set_dai_timestamp),
	SND_SOC_BYTES_EXT("SPDIF DAI Timestamp", BYTES_OF_INT64, aml_audio_get_spdif_dac_timestamp,
				aml_audio_set_dai_timestamp),
#endif
};
