...
/* 16kB should be enough to record */
#define HDCP_LOG_SIZE (1024 * 16)
struct hdcplog_buf {
	int idx;
	unsigned char buf[HDCP_LOG_SIZE + 64]; /* padding 64 bytes */
};
...
extern void cod_test(struct hdmitx_dev *hdev);
extern void cod_init(struct hdmitx_dev *hdev);
extern void cod_fini(struct hdmitx_dev *hdev);
 ...
 */
extern void hdmitx_audio_mute_op(unsigned int flag, unsigned int path);
