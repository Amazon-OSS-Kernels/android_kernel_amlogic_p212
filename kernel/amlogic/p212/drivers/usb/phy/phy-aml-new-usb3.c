...
static void disable_host_port(unsigned long reg_addr);
...
static char bc_mode_name[8] = "UNKNOW";

static int __init bc_mode_para_setup(char *s)
{
	if (NULL != s)
		sprintf(bc_mode_name, "%s", s);

	return 0;
}
__setup("BC_mode=", bc_mode_para_setup);

/**
 * Show the register offset of the Register Access.
 */
static ssize_t bc_mode_show(struct device *_dev,
 ...
 */
static ssize_t bc_mode_store(struct device *_dev,
			       struct device_attribute *attr,
			       const char *buf, size_t count)
{
	if (NULL != buf) {
		if (count < 8)
			sprintf(bc_mode_name, "%s", buf);
	}
	return count;
}

DEVICE_ATTR(bc_mode, S_IRUGO | S_IWUSR, bc_mode_show, bc_mode_store);

static char otg_mode;
static char otg_mode_name[8] = "HOST";
static int __init otg_mode_para_setup(char *s)
{
	if (NULL != s)
		sprintf(otg_mode_name, "%s", s);

	if (strcmp(otg_mode_name, "HOST") == 0)
		otg_mode = 0;
	else
		otg_mode = 1;
	return 0;
}
__setup("OTG_mode=", otg_mode_para_setup);
...
void set_host_mode(void)
{
	...
	/*enable vbus power when host mode for stark platform*/
...
static int amlogic_new_usb3_probe(struct platform_device *pdev)
{
	...
	device_create_file(&pdev->dev, &dev_attr_bc_mode);
...
{
	device_remove_file(&pdev->dev, &dev_attr_bc_mode);
