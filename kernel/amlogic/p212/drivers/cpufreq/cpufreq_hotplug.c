...
*/
#define AM905X_MAX_CPUFREQ      1512000
#define AM905X_OPT_CPUFREQ      1000000
...
static void hg_check_cpu(int cpu, unsigned int max_load)
{
	...
	if ((max_load_freq <
		...
		/* when there's no thermal throttling, do not go lower than OPT_CPUFREQ
		 */
		if (policy->max == AM905X_MAX_CPUFREQ && freq_next < AM905X_OPT_CPUFREQ)
            freq_next = AM905X_OPT_CPUFREQ;
        else
			freq_next = max_load_freq /	(hg_tuners->down_threshold);
