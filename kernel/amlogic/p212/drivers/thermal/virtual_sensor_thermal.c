 ...
 * @weight: Weight
 */
struct virtual_thermal_dev_params {
	int offset;
	int alpha;
	int weight;
};
...
/**
 * struct virtual_thermal_dev  - Structure for each thermal device.
 * @dev: Device node
 ...
 *           agents.
 * @data: the data that passed back through dev_ops
...
struct virtual_thermal_dev {
	struct device *dev;
	const char *name;
	struct virtual_thermal_dev_ops *dev_ops;
	int data;
	struct list_head node;
	struct virtual_thermal_dev_params tdp;
	long off_temp;
	long last_tempv;
};
...
static int virtual_sensor_get_temp(void *data, long *t)
{
	struct virtual_thermal_dev *tdev;
	int ret;
	long temp = 0;
	long tempv = 0;
	...
	list_for_each_entry(tdev, &vtz->virtual_sensor_dev_list, node) {
		...
		if (ret != 0){
			pr_info("%s virtual thermal zone %s device %s get temp return %d, use last temp reading %ld.\n", 
...
/* can get from:
   cat sys/bus/platform/devices/virtual-sensor.50/params
*/
...
/* Usage:
   cd sys/bus/platform/devices/virtual-sensor.50
...
static int virtual_sensor_parse_components(struct device_node *np, struct virtual_thermal_zone *vtz)
{
	...
	while ((child = of_get_next_child(np, child)) != NULL) {
		...
		ret |= of_property_read_u32(child, "offset", &offset);
		...
		list_for_each_entry(tdev, &vtz->virtual_sensor_dev_list, node) {
#ifdef CONFIG_PLATFORM_STARK_FEATURE
			//to support Stark hvt device with tmp
...
static int virtual_sensor_thermal_probe(struct platform_device *pdev)
{
	...
	struct virtual_thermal_dev *tdev, *tdev_new;
	...
	list_for_each_entry(tdev, &g_virtual_sensor_dev_list, node) {
		tdev_new = kzalloc(sizeof(struct virtual_thermal_dev), GFP_KERNEL);
		if (!tdev_new) {
			pr_err("%s: No memory\n", __func__);
			return -ENOMEM;
		}
...
{
	struct virtual_thermal_dev *vthermal_dev;
	...
	vthermal_dev = kzalloc(sizeof(struct virtual_thermal_dev), GFP_KERNEL);
	if (!vthermal_dev) {
		pr_err("%s: No memory\n", __func__);
		return -ENOMEM;
	}
