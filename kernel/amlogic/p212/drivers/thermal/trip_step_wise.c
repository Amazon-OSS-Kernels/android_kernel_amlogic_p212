...
static int trip_step_wise_throttle(struct thermal_zone_device *tz, int trip)
{
	...
	list_for_each_entry(tz_instance, &tz->thermal_instances, tz_node) {
		...
		pr_debug("thermal %s zone%d trip %d cdev %s trip_temp %d tz temp %d cur_state %d target %d instance-target %d instance name %s\n", 
		...
		if (cur_state != target) {
			...
			*/
			...
			if (new_state == target) {
				...
				snprintf(data[0], sizeof(data[0]), "THERMAL_STATE=%ld", thermal_state);
				snprintf(data[1], sizeof(data[1]), "ID=%d", tz->id);
				snprintf(data[2], sizeof(data[2]), "CDTYPE=%s", cdev->type);
				snprintf(data[3], sizeof(data[3]), "CDLEV=%d", new_state);
