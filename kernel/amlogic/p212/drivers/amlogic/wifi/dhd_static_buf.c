...
#define PREALLOC_WLAN_SEC_NUM		7
...
#define WLAN_SECTION_SIZE_12	(PREALLOC_WLAN_BUF_NUM * 512)
...
static struct wlan_mem_prealloc wlan_mem_array[PREALLOC_WLAN_SEC_NUM] = {
	...
	{NULL, (WLAN_SECTION_SIZE_8 + PREALLOC_WLAN_SECTION_HEADER)},
        {NULL, (WLAN_SECTION_SIZE_12 + PREALLOC_WLAN_SECTION_HEADER)},
};
...
void *bcmdhd_mem_prealloc(int section, unsigned long size)
{
	...
	if (section == WLAN_STATIC_DHD_WLFC_INFO) {
		...
		pr_info("5a %s: section=%d, wlan_mem_array[6]=%p\n",
			__func__, section, wlan_mem_array[6].mem_ptr);
		return wlan_mem_array[6].mem_ptr;
	}
