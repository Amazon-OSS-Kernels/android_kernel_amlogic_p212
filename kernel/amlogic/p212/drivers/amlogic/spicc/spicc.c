...
void spicc_set_mode(struct spicc *spicc, u8 mode)
{
...
void spicc_set_clk(struct spicc *spicc, int speed)
{
	...
	pr_info("%s speed=%d, sysclkrate=%d div=%d\n",
	        __func__, speed, sys_clk_rate, div);
}
...
static void dma_one_burst(struct spicc *spicc)
{
	...
	trace_spicc_dma_one_burst(spicc->remain);
	if (spicc->remain > 0) {
...
static void pio_one_burst(struct spicc *spicc)
{
	void __iomem *mem_base = spicc->regs;
	int bytes_per_word;
	unsigned int dat;
...
static int spicc_dma_xfer(struct spicc *spicc, struct spi_transfer *t)
{
	...
	setb(mem_base, STA_XFER_COM, 1); //write 1 clear
	...
	ret = wait_for_completion_interruptible_timeout(
		&spicc->completion,
		msecs_to_jiffies((t->len << 14)/spicc->speed + 10 + 2000));
	if (!ret) 
		printk(KERN_ERR "%s timed out", __func__);

	trace_spicc_dma_xfer_done(ret);
...
static int spicc_hw_xfer(struct spicc *spicc, u8 *txp, u8 *rxp, int len)
{
	...
	spicc->txp = txp; //(u8 *)t->tx_buf;
	spicc->rxp = rxp; //(u8 *)t->rx_buf;
	spicc->burst_len = 0;
	pio_one_burst(spicc);
	wait_for_completion_interruptible_timeout(
		&spicc->completion,
		msecs_to_jiffies((len << 14)/spicc->speed + 10));
...
static void spicc_hw_init(struct spicc *spicc)
{
	...
	setb(mem_base, CON_XCH, 0);
	setb(mem_base, CON_SMC, 0);  /* 0-dma, 1-pio */
	setb(mem_base, CON_SS_CTL, spicc_get_flag(spicc, FLAG_SSCTL));
	setb(mem_base, CON_DRCTL, 0);
	spicc_enable(spicc, 1);
	writel((0x00<<26 | // [ 6]Burst Number
			0x00<<20 | // [ 6]Thread ID
			0x00<<19 | // [ 1]Urgent
			spicc->dma_num_per_write_burst<<15 |  // [ 4]Number in one Write request burst(0:1,1:2...)
			spicc->dma_num_per_read_burst <<11 |  // [ 4]Number in one Read request burst(0:1,1:2...)
			spicc->dma_rx_threshold<<6 |  // [ 5]RxFIFO threshold(>rxth, will request write)
			spicc->dma_tx_threshold<<1 |  // [ 5]TxFIFO threshold(<txth, will request read)
			0x0<<0),   // [ 1]DMA Disable
...
int spicc_setup(struct spi_device *spi)
{
	struct spicc *spicc;
...
void spicc_handle_one_msg(struct spicc *spicc, struct spi_message *m)
{
	...
	if (spi) {
		...
		spicc_chip_select(spi, 1); // enable cs
	}
...
#else //SPICC_QUEUE_WORK

static int spicc_transfer_one_message(
		...
		struct spi_message *m)
{
	...
	trace_spicc_transfer(m);
	spicc_handle_one_msg(spicc, m);
	ret = m->status; // Need to save status once finalized this might change
...
int spicc_class_register(struct spicc *spicc)
{
	extern struct class_attribute spicc_class_attrs[];
	int ret;

	spicc->cls.name = kzalloc(10, GFP_KERNEL);
	sprintf((char *)spicc->cls.name, "spicc%d", spicc->master->bus_num);
...
static int spicc_probe(struct platform_device *pdev)
{
	...
	/*setup class*/
	ret = spicc_class_register(spicc);
	if (ret < 0)
