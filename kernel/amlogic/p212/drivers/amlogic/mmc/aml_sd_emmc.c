...
#define MULTI_DEVICE (0)

#if MULTI_DEVICE
#define aml_spin_lock_irqsave(lock, flags)      spin_lock_irqsave(lock, flags)
#define aml_spin_unlock_irqrestore(lock, flags) spin_unlock_irqrestore(lock, flags)
#else
#define aml_spin_lock_irqsave(lock, flags)      {}
#define aml_spin_unlock_irqrestore(lock, flags) {}
#endif
...
tunning:
	aml_spin_lock_irqsave(&host->mrq_lock, flags);
	pdata->need_retuning = false;
	aml_spin_unlock_irqrestore(&host->mrq_lock, flags);
