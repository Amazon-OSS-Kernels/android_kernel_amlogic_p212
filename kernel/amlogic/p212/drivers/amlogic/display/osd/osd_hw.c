...
void osd_init_hw(u32 logo_loaded)
{
	...
	*/
	...
	if (!timeline_created) { /* timeline has not been created */
		cpumask_t cpu_mask;
		// Same priority with EventThread
		...
		if (!IS_ERR_OR_NULL(buffer_toggle_thread)) {
			sched_setscheduler_nocheck(buffer_toggle_thread, SCHED_FIFO, &param);
			cpumask_clear(&cpu_mask);
			cpumask_set_cpu(2, &cpu_mask);
			cpumask_set_cpu(3, &cpu_mask);
			sched_setaffinity(buffer_toggle_thread->pid, &cpu_mask);
		}
		timeline_created = 1;
	}
	osd_init_hw_flag = 1;
