...
static void tv_out_disable_cvbs_gate(void)
{
	tv_out_hiu_set_mask(HHI_GCLK_OTHER,
		(0<<DAC_CLK) | (0<<GCLK_VENCI_INT) |
		(0<<VCLK2_VENCI1) | (0<<VCLK2_VENCI));
	tv_out_hiu_set_mask(HHI_VID_CLK_CNTL2,
		(0<<ENCI_GATE_VCLK) | (0<<VDAC_GATE_VCLK));
}
...
#endif
	if ((mode == TVMODE_576CVBS) || (mode == TVMODE_480CVBS))
