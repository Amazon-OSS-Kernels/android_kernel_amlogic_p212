...
#ifdef CONFIG_AVS
int scpi_osc_ring_val_set(u32 val)
{
	struct scpi_data_buf sdata;
	struct mhu_data_buf mdata;
	int stat;
	...
	SCPI_SETUP_DBUF(sdata, mdata, SCPI_CL_NONE,
			SCPI_CMD_OSC_RING_SET, val, stat);
	return scpi_execute_cmd(&sdata);
}
EXPORT_SYMBOL_GPL(scpi_osc_ring_val_set);


int scpi_get_avs_info(u32 val)
{
	struct scpi_data_buf sdata;
	struct mhu_data_buf mdata;
	struct __packed {
		...
		unsigned char ldd_cpu;
		unsigned char ldd_ee;
		unsigned char osc0_low;
		unsigned char osc1_low;
		unsigned char osc2_low;
		unsigned char osc3_low;
		unsigned char ring_high;
		unsigned char ring_low;
		unsigned char avs_default_delt;
		unsigned char osc_ring_avs_array;
		unsigned char dvfs_cnt;
		unsigned char avs_delt[MAX_DVFS_OPPS];
	} avsbuf;
	struct scpi_opp_entry *opp;
	unsigned int i, opp_len;
	int ret;

	SCPI_SETUP_DBUF(sdata, mdata, SCPI_CL_NONE,
			SCPI_CMD_GET_AVS_INFO, val, avsbuf);
	ret = scpi_execute_cmd(&sdata);
	pr_info("[avs_info] avs.soc_category=%d (1->5: FF->SS)\n", avsbuf.osc_ring_avs_array + 1);
	pr_info("[avs_info] avs.enable=%d\n", avsbuf.enable);
	pr_info("[avs_info] avs.version=%d\n", avsbuf.version);
	pr_info("[avs_info] avs.configs=%d\n", avsbuf.configs);
	pr_info("[avs_info] avs.Idd_cpu=%d.%d\n", (avsbuf.ldd_cpu*2)/10, (avsbuf.ldd_cpu*2)%10);
	pr_info("[avs_info] avs.Idd_ee=%d.%d\n", (avsbuf.ldd_ee*2)/10, (avsbuf.ldd_ee*2)%10);
	pr_info("[avs_info] avs.osc0_low=%d\n", avsbuf.osc0_low);
	pr_info("[avs_info] avs.osc1_low=%d\n", avsbuf.osc1_low);
	pr_info("[avs_info] avs.osc2_low=%d\n", avsbuf.osc2_low);
	pr_info("[avs_info] avs.osc3_low=%d\n", avsbuf.osc3_low);
	pr_info("[avs_info] osc_ring.high=%d\n", avsbuf.ring_high);
	pr_info("[avs_info] osc_ring.low=%d\n", avsbuf.ring_low);
	pr_info("[avs_info] avs_default_delt=%d\n", avsbuf.avs_default_delt);

	opp_len = avsbuf.dvfs_cnt;
	opp = scpi_opps[0]->opp;
	for (i = 0; i < opp_len; i++, opp++) {
		pr_info("[avs_info]:DVFS(%d, %d, %d)\n",
			opp->freq_hz,
			opp->volt_mv-avsbuf.avs_delt[i]*10,
			avsbuf.avs_delt[i]);
	}
	return ret;
}
EXPORT_SYMBOL_GPL(scpi_get_avs_info);
#endif
