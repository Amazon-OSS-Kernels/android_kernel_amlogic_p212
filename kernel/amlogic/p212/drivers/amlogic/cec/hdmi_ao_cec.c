...
int cec_ll_rx(unsigned char *msg, unsigned char *len)
{
	...
	*len = aocec_rd_reg(CEC_RX_MSG_LENGTH) + 1;
	...
	/* clear last message so that back-to-back non-retry commands can be sent */
	memset(last_cec_msg, 0, sizeof(*last_cec_msg));
...
int cec_ll_tx(const unsigned char *msg, unsigned char len)
{
	...
	/*
	 * should not retry for last NACK msg in AOCEC
	 * it's hardware will do this.
	 */
...
try_again:
	...
	if (!ee_cec) {
		...
		if (ret == CEC_FAIL_NACK) {
			memcpy(last_cec_msg->msg, msg, min(len, sizeof(last_cec_msg->msg)));
			last_cec_msg->len = min(len, sizeof(last_cec_msg->msg));
		}
	}
	return ret;
}
...
void cec_key_report(int suspend)
{
	...
	input_sync(cec_dev->cec_info.remote_cec_dev);
	if (!suspend)
		pr_info("== WAKE UP BY CEC ==\n");
	else
		pr_info("== SLEEP by CEC==\n");
}
...
void cec_set_osd_name(int dest)
{
	...
	if (0xf != dest) {
		...
		memcpy(&msg[2], cec_dev->cec_info.osd_name, min(osd_len, sizeof(msg)-2));
...
static void cec_rx_process(void)
{
	...
	/* Update physical address in case user plugs out/in during standby */
	if (cec_dev->dev_type == DEV_TYPE_PLAYBACK && !phy_addr_test) {
		/* physical address for mbox */
		if (cec_dev->tx_dev->hdmi_info.vsdb_phy_addr.valid == 1) {
	...
	/* Update address if it is valid otherwise continue using older one
	...
	memcpy(msg, rx_msg, min(len, sizeof(msg)));
	...
	case CEC_OC_ROUTING_CHANGE:
		...
		pr_info("cec_rx_process: wake up on <routing change> check: phy_addr=0x%x, dest_phy_addr=0x%x\n", cec_dev->phy_addr, dest_phy_addr);
		if (dest_phy_addr == cec_dev->phy_addr) {
			pr_info("cec_rx_process: wake up by ROUTING_CHANGE\n");
			cec_key_report(0);
		}
		break;
	...
	case CEC_OC_GIVE_PHYSICAL_ADDRESS:
		pr_info("cec_rx_process: cec_dev->phy_addr=0x%x\n", cec_dev->phy_addr);
	...
	case CEC_OC_SET_STREAM_PATH:
		...
		/* wake up if in early suspend and <set stream path> is directed to us */
		pr_info("cec_rx_process: wake up on <set stream path> check: phy_addr=0x%x, dest_phy_addr=0x%x\n", cec_dev->phy_addr, dest_phy_addr);
		if ((cec_dev->cec_suspend == CEC_EARLY_SUSPEND) && (dest_phy_addr == cec_dev->phy_addr)) {
			pr_info("cec_rx_process: waking up on <set stream path>");
			cec_key_report(0);
	    }
		break;
	...
	case CEC_OC_USER_CONTROL_PRESSED:
		...
		if (cec_dev->cec_suspend == CEC_EARLY_SUSPEND) {
			if (msg[2] == 0x40 || msg[2] == 0x6d) {
				pr_info("cec_rx_process: waking up on <user control pressed <power>>");
				cec_key_report(0);
			}
		}
		break;
