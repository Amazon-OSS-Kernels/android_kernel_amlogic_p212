...
static bool meson_cpufreq_boost = false;
...
int meson_cpufreq_enable_boost(bool enable)
{
	...
	if (enable) {
		...
		pr_err("cpufreq_boost enabled\n");
	} else {
		meson_cpufreq_boost = false;
		pr_err("cpufreq_boost disabled\n");
	}
	return 1;
}
...
#ifdef CONFIG_AVS
static ssize_t show_avs(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	scpi_get_avs_info(0x0);
	return 0;
}

static ssize_t enable_avs(struct device *dev, struct device_attribute *attr,
			const char *buf, size_t count)
{
	int osc_ring, buf_len;

	buf_len = strlen(buf);
	if (((buf[0] == '0') || (buf[0] == '1')) && (buf_len == 2)) {
			osc_ring = buf[0] - '0';
			osc_ring = osc_ring ? 0 : 1;
			scpi_osc_ring_val_set(osc_ring);
		} else {
			pr_err("echo avs value err");
	}
	return count;
}
static DEVICE_ATTR(avs, 0644, show_avs, enable_avs);
#endif
...
#ifdef CONFIG_AVS
	ret = device_create_file(cpu_dev, &dev_attr_avs);
	if (ret < 0)
		dev_err(cpu_dev, "create avs_info sysfs failed\n");
#endif
	meson_freq_table[idx].driver_data = idx;
