...
int codec_mm_scatter_get_cached_size(int is_tvp)
{
	int size = codec_mm_get_scatter_mgt(is_tvp)->cached_pages;
	return size << PAGE_SHIFT;
}

int codec_mm_scatter_get_alloced_size(int is_tvp)
{
	struct codec_mm_scatter_mgt *smgt =
		codec_mm_get_scatter_mgt(is_tvp);
	int size;
	codec_mm_list_lock(smgt);
	size = smgt->alloced_page_num;
	size -= smgt->cached_pages;
	codec_mm_list_unlock(smgt);
	return size << PAGE_SHIFT;
}
