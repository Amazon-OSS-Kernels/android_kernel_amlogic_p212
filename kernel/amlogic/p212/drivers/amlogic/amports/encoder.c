...
#define MAX_ADDR_INFO_SIZE 50
	...
	case AMVENC_AVC_IOC_FLUSH_CACHE:
        ...
        if ((buf_start + addr_info[1]) > (wq->mem.buf_start + wq->mem.buf_size -1) ||
            (buf_start + addr_info[1]) < wq->mem.buf_start) {
            enc_pr(LOG_ERROR, "avc cache flush addr_info[1] out of range\n");
            return -1;
        }
        if ((buf_start + addr_info[2]) > (wq->mem.buf_start + wq->mem.buf_size -1) ||
            (buf_start + addr_info[2]) < wq->mem.buf_start) {
            enc_pr(LOG_ERROR, "avc cache flush addr_info[2] out of range\n");
            return -1;
        }
        if (addr_info[2] < addr_info[1]) {
            enc_pr(LOG_ERROR, "avc cache flush, addr_info[2] less than addr_info[1]\n");
            return -1;
        }
	...
	case AMVENC_AVC_IOC_FLUSH_DMA:
        ...
        if ((buf_start + addr_info[1]) > (wq->mem.buf_start + wq->mem.buf_size -1) ||
            (buf_start + addr_info[1]) < wq->mem.buf_start) {
            enc_pr(LOG_ERROR, "avc dma flush addr_info[1] out of range\n");
            return -1;
        }
        if ((buf_start + addr_info[2]) > (wq->mem.buf_start + wq->mem.buf_size -1) ||
            (buf_start + addr_info[2]) < wq->mem.buf_start) {
            enc_pr(LOG_ERROR, "avc dma flush addr_info[2] out of range\n");
            return -1;
        }
        if (addr_info[2] < addr_info[1]) {
            enc_pr(LOG_ERROR, "avc dma flush, addr_info[2] less than addr_info[1]\n");
            return -1;
        }
	...
	case AMVENC_AVC_IOC_READ_CANVAS:
        ...
        if ((dst.addr + dst.width * dst.height * 3 / 2) > (wq->mem.buf_start + wq->mem.buf_size -1) ||
            dst.addr < wq->mem.buf_start) {
            enc_pr(LOG_ERROR, "avc canvas dma flush out of range\n");
            return -1;
        }
