...
int output_frames(struct h264_dpb_stru *p_H264_Dpb, unsigned char flush_flag)
{
	...
	int dpb_buf_size = p_H264_Dpb->reorder_pic_num;

	if ((p_H264_Dpb->bitstream_restriction_flag) &&
		(p_H264_Dpb->max_dec_frame_buffering < dpb_buf_size))
		dpb_buf_size = p_H264_Dpb->max_dec_frame_buffering;
	...
	if (!flush_flag) {
		for (i = 0; i < p_Dpb->used_size; i++) {
			if ((!p_Dpb->fs[i]->is_output) &&
				...
				if ((p_H264_Dpb->fast_output_enable & 0x1) &&
					...
					!p_H264_Dpb->fast_output_see_idr &&
					(p_H264_Dpb->decode_pic_count == 1)) {
					fast_output_flag = 1;
					p_H264_Dpb->fast_output_see_idr = true;
				}
		...
		if (fast_output_flag)
			;
		else if (none_displayed_num < dpb_buf_size)
			return 0;
	}
