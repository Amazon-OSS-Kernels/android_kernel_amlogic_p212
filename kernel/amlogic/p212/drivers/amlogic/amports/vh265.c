...
#define OVER_SIZE(w, h)		(HEVC_SIZE < (w + h))
...
static struct vframe_s *vh265_vf_peek(void *op_arg)
{
	...
	if (kfifo_out_peek(&hevc->display_q, &vf, 2)) {
		if (vf[1]) {
...
static int vh265_local_init(struct hevc_state_s *hevc)
{
	...
	pts_unstable = ((unsigned long)hevc->vh265_amstream_dec_info.param
		& 0x40) >> 6;
	hevc_print(hevc, 0,
		"h265:pts_unstable=%d lowlatency_flag= %d\n", pts_unstable, hevc->lowlatency_flag);
/*
TODO:FOR VERSION
*/
	hevc_print(hevc, 0,
...
#endif
		return;
	} else if (hevc->uninit_list == 2) {
		pr_info("vh265_work after uninit_list %d\n", hevc->dec_result);
		return;
	}
