...
static int omx_pts_interval_upper = 1440;
static int omx_pts_interval_lower = -1440;
...
static void log_vsync_video_pattern(int pattern) {
	int factor1, factor2, pattern_range;
...
#endif
{
	...
	if (omx_secret_mode == true) {
		...
		if ((diff - omx_pts_interval_upper) > 0
			...
			|| omx_pts_set_from_hwc_count < OMX_MAX_COUNT_RESET_SYSTEMTIME) {
...
static void set_omx_pts(u32 *p)
{
	...
	if (debug_flag & DEBUG_FLAG_OMX_DEBUG_DROP_FRAME) {
		pr_info("check %d,session 0x%x,"
				...
				"omx_index %d,pts %d,need_drop %d\n",
			omx_check_previous_session,
			session, omx_cur_session,
			frame_num, tmp_pts, !set_from_hwc);
	}
