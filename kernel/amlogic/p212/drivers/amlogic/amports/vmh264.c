...
#define H264_VDEC_INIT_FIFO_LEVEL 0x800
...
static unsigned int min_active_buffer_size = 13;
...
*/
static unsigned int error_proc_policy = 0x14; /*0xc;*/
...
#define BMMU_MAX_BUFFERS	(BUFSPEC_POOL_SIZE + 2)
...
int prepare_display_buf(struct vdec_s *vdec, struct FrameStore *frame)
{
	...
	if (display_frame_count[DECODE_ID(hw)] == 0)
		pr_info("avc decoder first frame ready\n");
...
pr_info("get_max_dec_frame_buf_size level_idc = %d, mb %dx%d\n", level_idc, mb_width, mb_height);
	switch (level_idc) {
	case 9:
		size = 152064;
		break;
...
static int vh264_set_params(struct vdec_h264_hw_s *hw)
{
	...
	if (hw->config_bufmgr_done == 0) {
		...
		pr_info("restriction_flag=%d, max_dec_frame_buffering=%d, reorder_pic_num=%d\n",
			hw->bitstream_restriction_flag,
			hw->max_dec_frame_buffering,
			hw->dpb.reorder_pic_num);

		if ((hw->bitstream_restriction_flag) &&
			(hw->max_dec_frame_buffering < hw->dpb.reorder_pic_num))
			hw->dpb.reorder_pic_num = hw->max_dec_frame_buffering;
		active_buffer_spec_num =
			hw->dpb.reorder_pic_num
			+ used_reorder_dpb_size_margin;
		if (active_buffer_spec_num < min_active_buffer_size)
			active_buffer_spec_num = min_active_buffer_size;
...
static void vui_config(struct vdec_h264_hw_s *hw)
{
	...
	hw->bitstream_restriction_flag = p_H264_Dpb->bitstream_restriction_flag;
	hw->num_reorder_frames = p_H264_Dpb->num_reorder_frames;
	hw->max_dec_frame_buffering = p_H264_Dpb->max_dec_frame_buffering;

	/*pr_debug("vui_config: pdb %d, %d, %d\n",
...
static bool is_buffer_available(struct vdec_s *vdec)
{
	...
	if ((kfifo_len(&hw->newframe_q) <= 0) ||
		...
		dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_DETAIL,
		"%s => 1, newq(%d), free_spec(%d), initdon(%d), used_size(%d/%d), unused_fr_dpb(%d)\n",
		__func__,
		kfifo_len(&hw->newframe_q),
		have_free_buf_spec(vdec),
		p_H264_Dpb->mDPB.init_done,
...
static irqreturn_t vh264_isr(struct vdec_s *vdec)
{
	...
	if (dec_dpb_status == H264_CONFIG_REQUEST) {
		...
		pr_info("H264_CONFIG_REQUEST: pdb %d, %d, %d\n",
			p_H264_Dpb->bitstream_restriction_flag,
			p_H264_Dpb->num_reorder_frames,
			p_H264_Dpb->max_dec_frame_buffering);
			...
			if (error_proc_policy & 0x4) {
				...
				if ((currSlice->slice_type != I_SLICE) &&
					...
					for (i = 0; i < currSlice->listXsize[0]; i++) {
						if ((currSlice->listX[0][i] != NULL) && currSlice->listX[0][i]->frame &&
				...
				if (currSlice->slice_type == B_SLICE) {
					for (i = 0; i < currSlice->listXsize[1]; i++) {
						if ((currSlice->listX[1][i] != NULL) && currSlice->listX[1][i]->frame &&
...
static int dec_status(struct vdec_s *vdec, struct vdec_info *vstatus)
{
	...
	vstatus->status = hw->stat | hw->fatal_error;
...
static bool run_ready(struct vdec_s *vdec)
{
	...
	if (vdec_stream_based(vdec) && (hw->init_flag == 0)) {
		...
		if (level < H264_VDEC_INIT_FIFO_LEVEL)
			return 0;
	}
...
int ammvdec_h264_mmu_init(struct vdec_h264_hw_s *hw)
{
	...
	pr_info("ammvdec_h264_mmu_init tvp = 0x%x\n", tvp_flag);
...
module_param(min_active_buffer_size, uint, 0664);
MODULE_PARM_DESC(min_active_buffer_size, "\n ammvdec_h264 min_active_buffer_size\n");
