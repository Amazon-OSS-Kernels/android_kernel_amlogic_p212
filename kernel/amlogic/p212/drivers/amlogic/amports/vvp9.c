...
#define STAT_VDEC_VP9_TRIGGER_DONE						0x100
...
#define VP9_LOG_DECODING                 0x4000000
...
static void config_mpred_hw(struct VP9Decoder_s *pbi)
{
	...
	if ((debug & VP9_DEBUG_BUFMGR) || (debug & VP9_LOG_DECODING))
...
static struct vframe_s *vvp9_vf_peek(void *op_arg)
{
	...
	if (kfifo_out_peek(&pbi->display_q, &vf, 2)) {
		if (vf[1]) {
				...
				struct PIC_BUFFER_CONFIG_s *pic_config)
{
	...
	if (vf) {
		...
		vf->mem_handle = decoder_bmmu_box_get_mem_handle(
			...
			VF_BUFFER_IDX(pic_config->index));
#endif
		if (!(pic_config->y_crop_width == 196 && pic_config->y_crop_height == 196)) {
			...
			VFRAME_EVENT_PROVIDER_VFRAME_READY, NULL);
						...
						pbi->stat |= STAT_VDEC_VP9_TRIGGER_DONE;
            pr_info("[%s %d] drop trigger frame width %d height %d status %x\n", __func__, __LINE__,vf->width,vf->height, pbi->stat);
        }
	}
	...
	*/
	...
	if ((dec_status == HEVC_NAL_DECODE_DONE) ||
		...
		if (pbi->m_ins_flag) {
			...
			if (debug & VP9_LOG_DECODING)
				pr_info("vp9 isr dec status  = 0x%x, lcu 0x%x shiftbyte 0x%x\n",
...
static int vvp9_stop(struct VP9Decoder_s *pbi)
{
	...
	if (pbi->stat & STAT_VDEC_VP9_TRIGGER_DONE)
		pbi->stat &= ~STAT_VDEC_VP9_TRIGGER_DONE;
...
static void vp9_work(struct work_struct *work)
{
	...
	if (((pbi->dec_result == DEC_RESULT_GET_DATA) ||
		...
		if (pbi->last_timeout_num != pbi->timeout_num) {
			msleep(50);
			pbi->last_timeout_num = pbi->timeout_num;
			vp9_print(pbi, 0,
				"timeout delay. wait hw stable\n");
		}
#ifdef VP9_10B_MMU
		pbi->used_4k_num =
			(READ_VREG(HEVC_SAO_MMU_STATUS) >> 16);
		...
		if (debug & VP9_LOG_DECODING)
			vp9_print(pbi, 0,
				"%s (===> %d) dec_result %d dec status 0x%x lcu 0x%x shiftbyte 0x%x\n",
				__func__,
				pbi->frame_count,
				pbi->dec_result,
				pbi->dec_status,
	...
	void (*callback)(struct vdec_s *, void *), void *arg)
{
	...
	if (debug & VP9_LOG_DECODING) {
		vp9_print(pbi, 0,
			"%s (%d): size 0x%x sum 0x%x\n",
			__func__,
			pbi->frame_count, r,
			get_data_check_sum(pbi, r));
	}
	else if (debug & PRINT_FLAG_VDEC_STATUS) {
		int ii;
		vp9_print(pbi, 0,
