...
#define NEEDLE_HVT_BOARDID	10030017
#define NEEDLE_EVT_BOARDID	20030017
...
static void clear_hdcp22_exp(void);
...
/* Topology information for hdcp repeater */
static struct downstream_topo_info topoinfo;

int tx_debug;
static int hdr_send_block;
...
static void hdmitx_early_suspend(struct early_suspend *h)
{
	...
	if (info && (strncmp(info->name, "panel", 5) == 0
		...
		return;
	hdmi_print(IMP, SYS "HDMITX: early suspend return\n");
	return;
	suspend_flag = 1;
	phdmi->hpd_lock = 1;
	msleep(20);
	phdmi->HWOp.CntlMisc(phdmi, MISC_AVMUTE_OP, SET_AVMUTE); //TV menu kept popping up.
	mdelay(100);
	hdmi_print(IMP, SYS "HDMITX: early suspend\n");
	phdmi->HWOp.Cntl((struct hdmitx_dev *)h->param,
		HDMITX_EARLY_SUSPEND_RESUME_CNTL, HDMITX_EARLY_SUSPEND); //This causes no signal.
...
static void hdmitx_late_resume(struct early_suspend *h)
{
	...
	hdmi_print(INF, SYS "late resume return\n");
	return;
	hdcp_tst_sig = 0;
	if (info && (strncmp(info->name, "panel", 5) == 0 ||
		strncmp(info->name, "null", 4) == 0)) {
...
static int set_disp_mode_auto(void)
{
	 ...
	 * as two different modes, so if node "attr" contains 420,
	 * need appending 420 to mode.
	 */
	...
	struct device_attribute *attr, char *buf)
{
	...
	pos += snprintf(buf+pos, PAGE_SIZE, "%s", fmt_attr);
	return pos;
}
 ...
 */
static ssize_t show_hdcp_repeater(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	int pos = 0;
	struct hdmitx_dev *hdev = &hdmitx_device;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (hdev->hdcp_mode == 2) {
		...
		cnt = sscanf(buf, "%x %x %x %x %x %x",
					(int *)&topoinfo.max_devs_exceeded,
					(int *)&topoinfo.max_cascade_exceeded,
					(int *)&topoinfo.v2_0_repeater_down,
					(int *)&topoinfo.v1_X_device_down,
					(int *)&topoinfo.device_count,
					(int *)&topoinfo.depth);
		if (cnt < 0)
			return count;
		else {
			/* reformat data bit size, refer to hdcp22 spec P60 */
			topoinfo.max_devs_exceeded =
				!!topoinfo.max_devs_exceeded;
			topoinfo.max_cascade_exceeded =
				!!topoinfo.max_cascade_exceeded;
			topoinfo.v2_0_repeater_down =
				!!topoinfo.v2_0_repeater_down;
			topoinfo.v1_X_device_down = !!topoinfo.v1_X_device_down;
			topoinfo.device_count = topoinfo.device_count & 0x1f;
			topoinfo.depth = topoinfo.depth & 0x7;
		}
	}
...
/*
 * hdcp22_exp attr: for hdcp22 exception use only
 */
static ssize_t show_hdcp22_exp(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int pos = 0;
...
static ssize_t store_hdcp22_exp(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	if (buf[0] == '0')
		cnt_hdcp22_exp = 0;
...
static DEFINE_SEMAPHORE(aud_mute_lock);
#define AUDIO_MUTE_PATH_1 0x8000000
#define AUDIO_MUTE_PATH_2 0x4000000

void hdmitx_audio_mute_op(unsigned int flag, unsigned int path)
{
	static unsigned int aud_mute_path = 0;

	down_interruptible(&aud_mute_lock);
...
static void hdr_work_func(struct work_struct *work)
{
	...
	if (hdev->hdr_color_feature == C_BT709)
...
/*
   ...
   */
	...
	if (hdev->hdr_transfer_feature == T_BT709 && hdev->hdr_color_feature == C_BT2020) {
		hdmitx_device.HWOp.CntlConfig(&hdmitx_device, CONF_AVI_BT2020, SET_AVI_BT2020);
	}

	/*SDR*/
	if (hdev->hdr_transfer_feature == T_BT709 &&
		((hdev->hdr_color_feature == C_BT709) || (hdev->hdr_color_feature == C_BT2020))) {
#if 1 // #ifdef USE_TASK_FOR_DRM_OFF
		// only issue SDR DRM packets if there is a need to clear HDR DRM packets
		if (DRM_DB[0] == 0x02) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strncmp(buf, "force", 5) == 0)
		...
		if (strncmp(buf+6, "off", 3) == 0) {
			hdmitx_audio_mute_op(0, AUDIO_MUTE_PATH_1);
			hdmi_print(IMP, AUD "configure off\n");
		} else if (strncmp(buf+6, "on", 2) == 0) {
			hdmitx_audio_mute_op(1, AUDIO_MUTE_PATH_1);
	...
	} else if (strncmp(buf, "drm_", 4) == 0) {
		struct master_display_info_s mdi =
		{
			.features = 0,
			.primaries[0][0] = 0x33c2,
			.primaries[0][1] = 0x86c4,
			.primaries[1][0] = 0x1d4c,
			.primaries[1][1] = 0x0bb8,
			.primaries[2][0] = 0x84d0,
			.primaries[2][1] = 0x3e80,
			.white_point[0]  = 0x3d13,
			.white_point[1]  = 0x4042,
			.luminance[0]    = 0x044c,
			.luminance[1]    = 0x0000,
		};

		if (strncmp(buf+4, "on", 2) == 0) {
			mdi.features = (T_SMPTE_ST_2084 << 8) | (C_BT2020 << 16);
		}
		else if (strncmp(buf+4, "off", 3) == 0) {
			mdi.features = (T_BT709 << 8) | (C_BT709 << 16);
		}

		if (get_cpu_type() >= MESON_CPU_MAJOR_ID_GXTVBB)
			hdmitx_set_drm_pkt(&mdi);
	} else if (strncmp(buf, "hdr.sdr", 7) == 0) {
		struct hdmitx_dev *hdev = &hdmitx_device;
		unsigned char DRM_HB[3] = {0x87, 0x1, 26};
		unsigned char DRM_DB[26] = {0x0};
		unsigned long delay = 200; /* Unit ms, default delay 200ms */
		int ret;

		// delay of 0 effectively disables the block
		if (strlen(buf) >= 9 && isdigit(buf[8]) && strlen(buf) < 14)
			ret = kstrtoul(&buf[8], 10, &delay);

		hdr_send_block = delay ? 1 : 0;

		if (hdr_send_block == 0)
			return strlen(buf);

		// bypass DRM packet update if we are not in HDR mode at the moment
		if (hdev->hdr_transfer_feature != T_SMPTE_ST_2084 || hdev->hdr_color_feature != C_BT2020)
			return strlen(buf);

		hdev->HWOp.SetPacket(HDMI_PACKET_DRM, DRM_DB, DRM_HB);
		hdev->HWOp.CntlConfig(hdev, CONF_AVI_BT2020, CLR_AVI_BT2020);
		msleep(min(delay, 1000));
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	if (buf[0] == '1') {
		...
		hdmitx_audio_mute_op(0, AUDIO_MUTE_PATH_2);
	}
	if (buf[0] == '0') {
		if (!(atomic_sub_and_test(0, &kref_audio_mute))) {
			atomic_dec(&kref_audio_mute);
		}
		hdmitx_audio_mute_op(1, AUDIO_MUTE_PATH_2);
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	pos += snprintf(buf+pos, PAGE_SIZE, "%d, %d, %d \n",
		hdmitx_device.RXCap.Latency_Present, hdmitx_device.RXCap.Video_Latency,
		hdmitx_device.RXCap.Audio_Latency);
	return pos;
}

static ssize_t print_aud_cap(char * buf, struct rx_audiocap * cap, char * forced_name) {
	int j, pos = 0;
	...
	pos += snprintf(buf + pos, PAGE_SIZE, "%s, %d ch, ",
			!forced_name ? aud_coding_type[cap->audio_format_code] : forced_name,
			cap->channel_num_max + 1);
	...
	for (i = 0; i < pRXCap->AUD_count; i++) {
		...
		if (pRXCap->RxAudioCap[i].audio_format_code == 10)
			if (pRXCap->RxAudioCap[i].cc3 & 0x3)
				pos += print_aud_cap(&buf[pos], &pRXCap->RxAudioCap[i], "DolbyAtmos");
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strncmp(buf, "1", 1) == 0) {
		...
		edid_rx_data(0x0, tmp, sizeof(tmp)); // 565
	...
	struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&getedid_mutex);
	// Following check is disabled intentionally
	// to keep RX version check decoupled from TX caps
#if 0
...
{
	struct hdmitx_dev *hdev = &hdmitx_device;
	return snprintf(buf, PAGE_SIZE, "%d", hdmi_audio.state);
}
...
{
	struct hdmitx_dev *hdev = &hdmitx_device;
	return snprintf(buf, PAGE_SIZE, "%d", hdmi_audio.state && sdev.state && hdev->hpd_state);
}
...
static ssize_t store_sdr_hdr(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	if (strncmp(buf, "0", 1) == 0)
		hdmitx_device.sdr_hdr_feature = 0;
	if (strncmp(buf, "1", 1) == 0)
		hdmitx_device.sdr_hdr_feature = 1;
	if (strncmp(buf, "2", 1) == 0)
		hdmitx_device.sdr_hdr_feature = 2;
	return count;
}
...
static DEVICE_ATTR(hdcp_topo_info, S_IWUSR | S_IRUGO | S_IWGRP,
	show_hdcp_topo_info, store_hdcp_topo_info);
static DEVICE_ATTR(hdcp22_type, S_IWUSR | S_IRUGO | S_IWGRP,
	show_hdcp22_type, store_hdcp22_type);
static DEVICE_ATTR(hdcp22_exp, S_IWUSR | S_IRUGO | S_IWGRP,
	show_hdcp22_exp, store_hdcp22_exp);
...
static DEVICE_ATTR(hpd_ctrl, S_IWUSR | S_IRUGO | S_IWGRP, show_hpd_ctrl, store_hpd_ctrl);
static DEVICE_ATTR(fake_plug, S_IWUSR | S_IRUGO | S_IWGRP, show_fake_plug, store_fake_plug);
...
static int amhdmitx_probe(struct platform_device *pdev)
{
	...
	hdmitx_device.hdcp_mode = 0;
	hdmitx_device.ready = 0;
	hdmitx_device.frac_rate_policy = 1; /* 1.000 / 1.001 modes by default */
	hdmitx_device.rxsense_policy = 1;
	...
	ret = device_create_file(dev, &dev_attr_hdcp22_exp);
...
/*Get SARADC channel for current overdraw detection on Needle DVT & PVT*/
	ret = of_property_read_u32(pdev->dev.of_node, "cod_saradc_ch", &val);
	board_id = idme_get_board_long_rev();
	hdmitx_device.cod_ch = (!ret && board_id!=NEEDLE_HVT_BOARDID && board_id!=NEEDLE_EVT_BOARDID) ? val : ~0x0;
...
static int amhdmitx_remove(struct platform_device *pdev)
{
	...
	device_remove_file(dev, &dev_attr_hdcp22_exp);
