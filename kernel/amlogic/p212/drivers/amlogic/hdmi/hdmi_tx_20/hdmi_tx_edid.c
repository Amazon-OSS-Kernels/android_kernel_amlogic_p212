	...
	for (offset = 4 ; offset < End ; ) {
		...
		case HDMI_EDID_BLOCK_TYPE_VENDER:
			...
			if ((BlockBuf[offset] == 0x03) &&
				...
				if (count >= 9 && (BlockBuf[offset+7] & 0x80) && BlockBuf[offset+8] != 0x00 && BlockBuf[offset+8] != 0xFF)
					pRXCap->Video_Latency = (BlockBuf[offset+8] - 1) * 2;
				if (count >= 10 && (BlockBuf[offset+7] & 0x80) && BlockBuf[offset+9] != 0x00 && BlockBuf[offset+9] != 0xFF)
					pRXCap->Audio_Latency = (BlockBuf[offset+9] - 1) * 2;
			}
...
static void hdmitx_edid_set_default_aud(struct hdmitx_dev *hdev)
{
	...
	pRXCap->RxAudioCap[0].cc3 = 1; /* 16 bit */
}
 ...
 */
	...
	/* Consider EDID parsed as long as signature is found and modes are parsed */
	if (pRXCap->IEEEOUI == 0x0c03 && pRXCap->VIC_count) {
		hdmitx_device->edid_parsing = 1;
	}
	...
	if (pRXCap->Latency_Present)
		pr_info("hdmi lipsync video %d audio %d\n", pRXCap->Video_Latency, pRXCap->Audio_Latency);
	else
