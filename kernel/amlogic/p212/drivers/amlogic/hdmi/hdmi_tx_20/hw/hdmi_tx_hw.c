...
static char rptx_ksv_buf[MAX_KSV_LISTS * 5];
...
/* [  3] i2c_fast_mode: 0=standard mode; 1=fast mode. */
	...
	hdmitx_wr_reg(HDMITX_DWC_I2CM_SS_SCL_HCNT_0, 0xBF);
	hdmitx_wr_reg(HDMITX_DWC_I2CM_SS_SCL_LCNT_1, 0);
	hdmitx_wr_reg(HDMITX_DWC_I2CM_SS_SCL_LCNT_0, 0xEF);
...
static void set_phy_by_mode(unsigned int mode)
{
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_GXL) {
		...
		case 2: /* 2.97Gbps */
			...
			break;
		case 3: /* 1.485Gbps */
			hd_write_reg(P_HHI_HDMI_PHY_CNTL0, 0x33303252);
...
static void hdcp_ksv_store(unsigned char *dat, int no)
{
	...
	/* must check rptx_ksv_no to prevent leak */
	if (rptx_ksv_no >= MAX_KSV_LISTS * 5)
		return;
			...
			for (i = 0; i < size; i++) {
				if (i < HEADER) { /* BSTATUS & M0 */
					...
					hdcp_mKsvListBuf[i - HEADER] =
						hdmitx_rd_reg(
						HDMITX_DWC_HDCP_BSTATUS_0 + i);
					tmp_ksv_lists.lists[tmp_ksv_lists.no++] = hdcp_mKsvListBuf[i - HEADER];
				} else { /* SHA */
...
static void hdcptx_events_handle(unsigned long arg)
{
	...
	if (st_flag & (1 << 7)) {
		...
		if (tmp_ksv_lists.valid) {
			hdcp_ksv_store(tmp_ksv_lists.lists, tmp_ksv_lists.no);
		}
		get_hdcp_bstatus();
	...
	mod_timer(&hdev->hdcp_timer, jiffies + HZ / 10);
}
	...
	unsigned long argv)
{
	...
	struct downstream_topo_info *topo;
	unsigned int val;
...
/* argv = !!((hdmitx_rd_reg(TX_HDCP_MODE)) & (1 << 7)); */
	...
	case DDC_HDCP14_GET_TOPO_INFO:
		topo = (struct downstream_topo_info *)argv;
		...
		topo->device_count = val & 0x7f;
		topo->max_devs_exceeded = !!(val & 0x80);
		val = hdmitx_rd_reg(HDMITX_DWC_HDCP_BSTATUS_1);
		topo->depth = val & 0x7;
		topo->max_cascade_exceeded = !!(val & 0x8);
		hdmitx_set_reg_bits(HDMITX_DWC_A_KSVMEMCTRL, 0, 0, 1);
		return 1;
	default:
 ...
 */
	...
	// This goes hand in hand with /sys/module/am_vecm/parameters/sdr_mode setting
	// done by user space
	if ((hdev->hdr_color_feature == C_BT2020) ||
		(hdev->hdmi_info.vsdb_phy_addr.a && hdev->hdmi_info.vsdb_phy_addr.b
 ...
 */
	...
	hdmitx_set_reg_bits(HDMITX_DWC_FC_DATAUTO3, 0xe, 0, 4);
