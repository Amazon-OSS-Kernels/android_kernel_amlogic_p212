...
/*begin sdr color mapping lut*/
static unsigned int eotf_33_709_mapping[EOTF_LUT_SIZE] = {
	    0,   143,   231,   342,   478,   639,   826,  1040,
	 1280,  1549,  1847,  2173,  2528,  2914,  3329,  3776,
	 4253,  4762,  5303,  5875,  6481,  7119,  7790,  8494,
	 9233, 10005, 10811, 11652, 12528, 13439, 14385, 15367,
	16383
};
	...
	struct vframe_master_display_colour_s *master_info)
{
	if ((get_cpu_type() == MESON_CPU_MAJOR_ID_GXL) ||
		...
		/* oetf lut gamma*/
		set_vpp_lut(VPP_LUT_OETF,
			oetf_289_709_mapping,
			oetf_289_709_mapping,
			oetf_289_709_mapping,
			CSC_ON);

		/* xvycc matrix rgb2yuv */
		set_vpp_matrix(VPP_MATRIX_XVYCC,
			RGB2020_to_YUV2020l_coeff,
			CSC_ON);
	...
	if (vf != NULL) {
		...
		if ((null_vf_cnt == 0) || (null_vf_cnt == toggle_frame)) {
			...
			if (null_vf_cnt == toggle_frame)
				vpp_matrix_update(
					&fake_vframe, vinfo, CSC_FLAG_TOGGLE_FRAME);
			else if (null_vf_cnt == 0)
				vpp_matrix_update(
					&fake_vframe, vinfo, CSC_FLAG_CHECK_OUTPUT);
			last_vf = NULL;
			fg_vf_sw_dbg = 4;
		}
