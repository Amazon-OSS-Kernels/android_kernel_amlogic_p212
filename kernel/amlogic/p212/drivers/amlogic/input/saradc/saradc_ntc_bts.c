/*
 * drivers/amlogic/input/saradc/saradc_ntc_bts.c
 *
 * ADC NTC BTS driver
 *
...
static int get_hw_bts_temp(int index, unsigned int *temp)
{

	int ret = 0, i, ret_temp = 0, output;
	...
	while (i--)
	{
		ret_temp = get_adc_sample(0, Channel);
		if (ret_temp > 0) //FIXME
		{
			valid_temp = ret_temp;
			ret += ret_temp;
			pr_debug("[thermal_auxadc_get_data]: ret_temp=%d\n",ret_temp);
			break;
		}
		msleep(5);
	}
	...
	if(ret == 0)
	{
		pr_err("SARADC channel %d reading error\n",Channel );
		return -EINVAL;
	}

	ret = ret*v_pullup/adc_range; /* 82's ADC power */
	pr_debug("Channel = %d\n", Channel);
	pr_debug("saradc reading 0x%x, output mV = %d\n",ret_temp, ret);
...
int ntc_bts_get_hw_temp(int index, unsigned long *temp)
{
	...
	if (t_ret < -1000) { /* abnormal low temp */
		pr_info("[Power/BTS_Thermal] T_AP=%d\n", t_ret);
		ret = -EINVAL;
	}
	...
	if(metrics_cnt == METRICSCOUNT+ADC_NUM)
		metrics_cnt = 0;
#endif
...
/* sysfs to read temperature.
   Usage:
   p212:/ # cat sys/bus/platform/devices/thermistor0.48/temp
   channel 0 temperature 87
   p212:/ # cat sys/bus/platform/devices/thermistor1.49/temp
...
static int ntc_bts_read_temp(struct device *dev, int index, unsigned long *temp)
{
	...
	return ntc_bts_get_hw_temp(index, temp);
}
static struct virtual_thermal_dev_ops ntc_bts_fops = {
...
#ifdef CONFIG_IDME
/*EVT NEEDLE:20030017, STARK:20020017*/
#define EVT_BOARD_LONG_REV 20020017
#define PRIOR_EVT_PULLUP_R 39000
	extern unsigned int idme_get_board_long_rev(void);
	{
		unsigned int board_rev = idme_get_board_long_rev();

		pr_info("IDME board_rev = %d\n", board_rev);
		if (board_rev > 0 && board_rev < EVT_BOARD_LONG_REV ) {
			bts_channel_param[serial].g_RAP_pull_up_R = PRIOR_EVT_PULLUP_R;
		}
