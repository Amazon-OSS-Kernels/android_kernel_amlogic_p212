...
void handle_privacy_button_pressed(unsigned long pressed)
{
	...
	if (pw_data && priv_workq && !pressed && pw_data->cur_priv && !mode_flag) {
		/* exit priv mode right away */
		pr_info("%s:%u: exit priv mode now\n", __func__, __LINE__);
		if (amz_priv_trigger(0)) {
			pr_info("%s:%u exit priv failed\n", __func__, __LINE__);
		} else {
			pr_info("%s:%d:\n", __func__, __LINE__);
			event_handled = 1;
		}
		...
		if (io_status != key->status) {
			if (io_status) {
				dev_info(&kp->input->dev,
					 "key %d up\n", key->code);
#ifdef CONFIG_AMZ_PRIV
				/*EVT mute GPIO is using DFF to hold state, so up means pressed*/
				if (!ishvt(board_id)) {
					if (strcmp(key->name, "mute") == 0) {
							mode_flag = 1;
					}
				}
#endif
				if(strcmp(key->name, "mute") != 0 || ishvt(board_id)){
				...
				if(ishvt(board_id)){
					if (strcmp(key->name, "mute") == 0) {
						handle_privacy_button_pressed(0);
					}
				}
#endif
				if (strcmp(key->name, "mute") == 0)
...
#ifdef CONFIG_AMZ_PRIV
			/*ignore first time pressed due to blink caused by hw*/
			if(!ishvt(board_id) && firstpressed){
				if (strcmp(key->name, "mute") == 0) {
					handle_privacy_button_pressed(0);
				}
			}
...
static int gpio_key_probe(struct platform_device *pdev)
{
	...
	if(!ishvt(board_id)){
		mode_flag = gpio_get_value(pdata->key[0].pin);
		handle_privacy_button_pressed(0);
	}
