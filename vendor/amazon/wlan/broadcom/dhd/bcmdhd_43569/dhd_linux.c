...
fw_download_thread_func(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (kthread_should_stop())
	        ...
	        if (dhd->pub.dbus == NULL) {
                        DHD_ERROR(("%s: %d, dbus_info is Null.\n", __FUNCTION__, __LINE__));
                        return DBUS_ERR;
	        }
...
#ifdef CONFIG_BRCM_SET_SPECT
	int spect = 1;
#else
	int spect = 0;
#endif /* CONFIG_BRCM_SET_SPECT */
...
#ifdef CONFIG_NO_PII
	DHD_ERROR(("Firmware up: op_mode=0x%04x, MAC=XXX\n",
		dhd->op_mode));
#else
...
#ifdef CONFIG_NO_PII
	printf("Register interface [%s]  MAC: XXX\n\n", net->name);
#else
