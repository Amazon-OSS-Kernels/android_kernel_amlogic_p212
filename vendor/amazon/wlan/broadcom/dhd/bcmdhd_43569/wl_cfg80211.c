...
static s32
wl_quiet_start(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
const wl_event_msg_t *e, void *data);
static s32
wl_quiet_stop(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
const wl_event_msg_t *e, void *data);
...
static chanspec_t wl_cfg80211_get_shared_freq(struct wiphy *wiphy)
{
	...
	if ((err = wldev_ioctl_get(dev, WLC_GET_BSSID, &bssid, sizeof(bssid)))) {
		...
		return wl_ch_host_to_driver(bssidx, channel);
	}
...
#ifdef CONFIG_NO_PII
	if (cfg->rcc_enabled) {
		WL_ERR(("Connecting with XXX ssid XXX, len (%d) with rcc channels \n\n",
			ext_join_params->ssid.SSID_len));
	} else {
		WL_ERR(("Connecting with XXX ssid XXX, len (%d) channel=%d\n\n",
...
#else
	/* TODO: This may not be needed in versions 4+, if disconnected can be
	called with more parameters indicating locally or AP generated */
	WL_ERR(("FASTCONNECT - not broadcasting local disconnect "
			"for kernels >= 3.14\n"));
#endif
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0)) */
...
#ifdef CONFIG_NO_PII
	WL_ERR(("Disconnect STA : XXX scb_val.val %d\n", scb_val.val));
#else
	WL_ERR(("Disconnect STA : %s scb_val.val %d\n",
...
#ifdef CONFIG_NO_PII
					printk("wl_bss_connect_done succeeded with XXX\n");
#else
...
static void wl_init_event_handler(struct bcm_cfg80211 *cfg)
{
	...
	cfg->evt_handler[WLC_E_QUIET_START] = wl_quiet_start;
	cfg->evt_handler[WLC_E_QUIET_END] = wl_quiet_stop;
#ifdef DHD_LOSSLESS_ROAMING
...
void
wl_cfg80211_ch_switch_notify(struct net_device *dev, uint16 chanspec, struct wiphy *wiphy, uint16_t csacount)
{
	u32 freq;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION (3, 8, 0))
	struct cfg80211_chan_def chandef;
	...
	cfg80211_ch_switch_notify_cnt(dev, &chandef, csacount);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION (3, 5, 0) && (LINUX_VERSION_CODE <= (3, 7, \
	\
	0)))
	freq = chan_info.freq;
	...
	WL_ERR(("Channel switch notification for freq: %d chanspec: 0x%x csacount %04X\n", freq, chanspec, csacount));
	return;
}
...
void
wl_cfg80211_quiet_stop(struct net_device *dev, struct wiphy *wiphy, int quietTime)
{
	if (!wiphy) {
		WL_ERR(("wiphy is null\n"));
		return;
	}
	cfg80211_quiet_stop(dev, quietTime);
	return;
}
...
const wl_event_msg_t *e, void *data)
{
	...
	WL_DBG(("Enter\n"));
	if (data) {
		csacount = *(uint16_t*)data;
		//cntdata = *(uint8_t*)data;
		//WL_DBG(("csadata %02X\n", cntdata));
		//if (cntdata & 1)
			//csacount = (1<<8) | ((cntdata>>1) & 0x00FF);
	}
	...
	if (likely(cfgdev)) {
		...
		wl_cfg80211_ch_switch_notify(ndev, chanspec, wiphy, csacount);
	} else {
...
static struct timespec sQuietTime;
...
{
	int startTime=0;
	get_monotonic_boottime(&sQuietTime);
	startTime = sQuietTime.tv_sec * 1000 + sQuietTime.tv_nsec / 1000000;
	WL_DBG(("%s time: %d\n", __func__, startTime));
	//Do we need to tie this timer to an interface? need to check
	return 0;
}
...
const wl_event_msg_t *e, void *data)
{
	...
	struct wiphy *wiphy = NULL;
	struct timespec ts;
	int totalQuietTime = 0;
	int endtime=0;
	get_monotonic_boottime(&ts);

	totalQuietTime = (ts.tv_sec - sQuietTime.tv_sec) * 1000 + (ts.tv_nsec - sQuietTime.tv_nsec) / 1000000;
	endtime = ts.tv_sec * 1000 + ts.tv_nsec / 1000000;
	WL_DBG(("%s time: %dms total quiet time: %dms\n", __func__, endtime, totalQuietTime));
	if (likely(cfgdev)) {
		ndev = cfgdev_to_wlc_ndev(cfgdev, cfg);
		wiphy = bcmcfg_to_wiphy(cfg);
		wl_cfg80211_quiet_stop(ndev, wiphy, totalQuietTime);
	} else {
		WL_ERR(("cfgdev is null\n"));
		return -1;
	}
