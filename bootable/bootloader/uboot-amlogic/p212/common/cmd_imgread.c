...
static int do_image_read_kernel(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	printf("kernel_size=0x%x, ramdisk_size=0x%x, dtbSz=0x%x, actualBootImgSz=0x%x\n", kernel_size, ramdisk_size, dtbSz, actualBootImgSz);
...
static int do_image_read_pic(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    // Use native sizes 1080p and below. Rely on scaling of 1080p resource above that.
    char picNameTmp[64] = { 0 };
    const char* outputmode = getenv("outputmode");
    if (outputmode)
    {
        if (!strncmp("720", outputmode, 3) || !strncmp("576", outputmode, 3) || !strncmp("480", outputmode, 3))
        {
            snprintf(picNameTmp, sizeof(picNameTmp), "%s_%c%c%c", picName, outputmode[0], outputmode[1], outputmode[2]);
            picNameTmp[sizeof(picNameTmp)-1] = '\0';
            picName = picNameTmp;
        }
    }
#endif
    ...
    for (itemIndex = 0; itemIndex < pResImgHead->imgItemNum; ++itemIndex, ++pItem)
    {
            ...
            if (!strcmp(picName, pItem->name))
            {
                    ...
                    setenv("bootup_offset", env_data);

                    sprintf(env_data, "0x%x", itemSz);
                    setenv("bootup_size", env_data);
