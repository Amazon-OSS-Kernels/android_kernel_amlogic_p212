...
#define DMF 1000
#define NUM_BTS_SENSORS 2
#define THERMAL_MAX 57000
#define NORMAL_MAX 61000
...
int offset[NUM_BTS_SENSORS] = {20000, 19000};
int alpha[NUM_BTS_SENSORS]  = {15,    15};
int weight[NUM_BTS_SENSORS] = {700,   400};
long off_temp[NUM_BTS_SENSORS] = {0,  0};
...
UINT32 get_current_temperature()
{
	...
	for (i=0; i < NUM_BTS_SENSORS; i++)
	{
		...
		if (off_temp[i] == 0) off_temp[i] = temp - offset[i];
		else
		{
			off_temp[i] = alpha[i] * (temp - offset[i]) +
			           (DMF - alpha[i]) * off_temp[i];
			off_temp[i] /= DMF;
		}
		tempv += (weight[i] * off_temp[i])/DMF;
