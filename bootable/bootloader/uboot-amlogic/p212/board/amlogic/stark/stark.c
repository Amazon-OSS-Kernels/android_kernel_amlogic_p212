...
{
#ifdef CONFIG_DISABLE_ETH
	clrbits_le32(HHI_GCLK_MPEG1,1<<3);
...
static void board_i2c_set_pinmux(void){
	...
	clrbits_le32(PERIPHS_PIN_MUX_1, ((1<<18)|(1<<19)|(1<<20)));
	clrbits_le32(PERIPHS_PIN_MUX_2, ((1<<13)|(1<<14)));
	//enable I2C MASTER AO pins
	setbits_le32(PERIPHS_PIN_MUX_1, ((1<<12)|(1<<13)));
...
struct amlogic_usb_config g_usb_config_GXL_skt_h={
	CONFIG_GXL_XHCI_BASE,
	USB_ID_MODE_HARDWARE,
	NULL,//gpio_set_vbus_power, //set_vbus_power
	CONFIG_GXL_USB_PHY2_BASE,
...
//#endif// #ifdef CONFIG_AML_V2_FACTORY_BURN
	...
	/*Power on GPIOAO_2 for USB_PWR_EN=0*/
	clrbits_le32(P_AO_GPIO_O_EN_N, ((1<<2)|(1<<18)));

	/*Set GPIOH_5 output low*/
	clrbits_le32(P_PREG_PAD_GPIO1_EN_N, (1 << 25));
	clrbits_le32(P_PREG_PAD_GPIO1_O, (1 << 25));

#ifdef CONFIG_USB_XHCI_AMLOGIC
	board_usb_init(&g_usb_config_GXL_skt,BOARD_USB_MODE_HOST);
	board_usb_init(&g_usb_config_GXL_skt_h,BOARD_USB_MODE_CHARGER);
#endif /*CONFIG_USB_XHCI_AMLOGIC*/
	canvas_init();
#ifdef CONFIG_AML_VPU
	vpu_probe();
...
int board_late_init(void){
	...
	char hdmi_cmd[] = ""\
		"hdmitx hpd;"\
		"if itest ${hpd_state} == 1}; then "\
			"hdmitx get_preferred_mode;"\
			"hdmitx output $outputmode;" \
			"echo hpd on path, ${hdmimode} ${outputmode};" \
		"else "\
			"setenv outputmode null;"\
			"setenv hdmimode null;"\
			"echo hpd off path ${hdmimode} ${outputmode};" \
		"fi;";

	run_command(hdmi_cmd, 0);
#endif
...
#ifdef CONFIG_STARK_MULTI_DTB
int checkstarkdtb(char * name)
{
	char loc_name[64] = {0};
	...
	if (!idme_get_var_external("board_id", buf, sizeof(buf))) {
		printf("IDME board_id = %s\n",buf);
		if(!strcmp(buf,"2901000000020017"))	//Proto
			strcpy(loc_name, "gxl_starkproto_2g\0");
		else //hvt and onward
			strcpy(loc_name, "gxl_p212_2g\0");

		if(strcmp(buf,"2901000000020017") && strcmp(buf,"2901001010020017") && strcmp(buf,"2901001020020017")){	//D|PVT change not proto|HVT|EVT
			/*Set GPIODV_24 output low*/
			printf("adc dac change for D/Pvt\n");
			clrbits_le32(P_PERIPHS_PIN_MUX_2, ((1 << 7)| (1 << 16)));
			clrbits_le32(P_PERIPHS_PIN_MUX_1, (1 << 15));
			clrbits_le32(P_PREG_PAD_GPIO0_EN_N, (1 << 24));
			clrbits_le32(P_PREG_PAD_GPIO0_O, (1 << 24));
		}
	} else {
		/*blank emmc assuming post proto */
		strcpy(loc_name, "gxl_p212_2g\0");
	}
#endif
