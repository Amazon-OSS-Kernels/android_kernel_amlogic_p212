...
#ifndef __GXL_STARK_V1_H__
#define __GXL_STARK_V1_H__
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"active_slot=_a\0"\
        ...
        "rpmb_state=0\0"\
        "initargs="\
            "rootfstype=ramfs init=/init console=ttyS0,460800 no_console_suspend earlyprintk=aml-uart,0xc81004c0 ramoops.pstore_en=1 ramoops.record_size=0x8000 ramoops.console_size=0x80000 "\
            "\0"\
        "upgrade_check="\
            ...
            "\0"\
    "storeargs="\
            "setenv bootargs ${initargs} androidboot.selinux=${EnableSelinux} logo=${display_layer},loaded,${fb_addr},${outputmode} maxcpus=${maxcpus} vout=${outputmode},enable hdmimode=${hdmimode} androidboot.firstboot=${firstboot} androidboot.rpmb_state=${rpmb_state} jtag=${jtag}; "\
	"setenv bootargs ${bootargs} BC_mode=${BC_mode} OTG_mode=${OTG_mode} androidboot.hardware=amlogic androidboot.serialno=123456789ABCDEFG;"\
	"setenv bootargs ${bootargs} androidboot.slot_suffix=${active_slot} u-boot_verion=${ver};"\
            "\0"\
        "switch_bootmode="\
            "get_rebootmode;"\
	    ...
	    "else if test ${reboot_mode} = fastboot; then "\
            ...
            "\0" \
        "storeboot="\
            "jtagoff apao; jtagoff apee; jtagoff scpao; jtagoff scpee;"\
        ...
        "recovery_from_flash="\
            ...
            "\0"\
        "init_display="\
            "gpio set BOOT_13;osd open;osd clear;imgread pic logo ${logo_name} $loadaddr;bmp display $bootup_offset;bmp scale"\
            "\0"\
        "bcb_cmd="\
            "get_valid_slot;"\
            "\0"\
        "upgrade_key="\
             "if gpio input GPIOX_3; then echo detect upgrade key; sleep 1; fi;"\
             "if gpio input GPIOX_3; then echo detect upgrade key; sleep 1; fi;"\
             "if gpio input GPIOX_3; then echo detect upgrade key; sleep 1; fi;"\
             "if gpio input GPIOX_3; then "\
                "echo detect upgrade key; sleep 1; fastboot;"\
             "fi;"\
            "\0"\
        "thermal_check="\
            "read_vs_temp;"\
            "if itest ${tempvs} > ${max_vs_temp}; then "\
                "setenv logo_name hot; run init_display; setenv logo_name bootup;"\
                "while itest ${tempvs} > ${max_vs_temp}; do read_vs_temp; echo cool wait; sleep 1; done;"\
            "else fi;"\
            "\0"\
...
#define CONFIG_DDR4_CLK					1176  //MHz, for boards which use different ddr chip
/* DDR type setting
...
#define CONFIG_STARK_MULTI_DTB          1
/* ddr functions */
...
#define CONFIG_USE_NATIVE_IMG_SIZE 1
...
#define CONFIG_DEVICE_PRODUCT	"stark"
