...
#endif
    baud_para=(24000000/3/gd->baudrate) -1;

    baud_para &= UART_REG5_BAUD_MASK;

    /* write to the register */
    writel((readl(P_UART_REG5(port_base)) & ~UART_REG5_BAUD_MASK) | baud_para | UART_REG5_BAUD_XTAL | UART_REG5_BAUD_USE, P_UART_REG5(port_base));
}
